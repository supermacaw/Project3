...
public class Solver {
	...
	/*private void helperHelper(HashSet<Block>result, int i, int j, int dir, Tray myTray){ // don't think i need to check for same block, b/c not possible?
	...
	private void emptyCoordsAdjBlocksHelper(int row, int col, Tray myTray, HashSet<Block> blockSet){
		helperHelper(blockSet, row-1, col, 0, myTray);//up, down, left, right
		helperHelper(blockSet, row + 1, col, 1, myTray);//GOTTA CHECK IF THEY"RE OUTTA BOUNDSSSS, maybe check this funky coordinate business
		helperHelper(blockSet, row, col-1, 2, myTray);    	//coord system weird, top down
		helperHelper(blockSet, row, col+1, 3, myTray);
	}

	public HashSet <Block> emptyCoordsAdjBlocks(Tray myTray){
		HashSet<Block> result = new HashSet<Block>();//repeated block adds
	...
	/*private void helperHelper(ArrayList<Block>result, int i, int j, int dir, Tray myTray){ // don't think i need to check for same block, b/c not possible?
		//System.out.println("Before crashing, i and j were " + i + " " + j);
		if(i>myTray.lengthOfTray-1 || i < 0 || j > myTray.widthOfTray-1 || j < 0){
			//System.out.println("We get here when i and j are " + i + " " + j);
			return;
	...
	private void emptyCoordsAdjBlocksHelper(int row, int col, Tray myTray, ArrayList<Block> result){
		helperHelper(result, row-1, col, 0, myTray);//up, down, left, right
		helperHelper(result, row + 1, col, 1, myTray);//GOTTA CHECK IF THEY"RE OUTTA BOUNDSSSS, maybe check this funky coordinate business
		helperHelper(result, row, col-1, 2, myTray);    	//coord system weird, top down
		helperHelper(result, row, col+1, 3, myTray);
	}
	...
	public static void main(String[] args) {
		...
		while (configRdr.hasNext()) {
			...
			if (configRdr.lineNumber() == 1) {
				...
				try{//just to make sure that random ass trays don't mess it up
